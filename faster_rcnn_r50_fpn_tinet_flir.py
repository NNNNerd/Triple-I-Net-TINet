...
model = dict(
    ...
    neck=dict(
        ...
        out_channels=256,
        num_outs=5),
    igfw=dict(downsample_size=(64, 64)),
    # igfw=None,
    inter=1,
    intra=1,
    weighted_loss=1,
...
# dataset settings
dataset_type = 'FLIRA'
data_root = '/home/zhangy/data/FLIR_aligned/format/'
img_norm_cfg = dict(
    mean_list=([160.915, 163.213, 161.171], [136.095, 136.095, 136.095]),
    std_list=([56.314, 59.272, 63.472], [66.367, 66.367, 66.367]), to_rgb=True) #RGB
train_pipeline = [
...
checkpoint_config = dict(  # Checkpoint hook 的配置文件。执行时请参考 https://github.com/open-mmlab/mmcv/blob/master/mmcv/runner/hooks/checkpoint.py。
    interval=3)  # save checkpoint interval。
...
work_dir = 'work_dirs/faster_rcnn_r50_fpn_tinet/flir'