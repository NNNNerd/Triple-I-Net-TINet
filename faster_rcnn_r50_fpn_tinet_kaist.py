...
data_root = '/home/zhangyan22/data/KAIST_cleaned/'
img_norm_cfg = dict(
    mean_list=([90.45, 84.18, 74.68], [44.26, 42.34, 44.26]),
    std_list=([68.16, 65.19, 66.07], [28.85, 28.70, 28.85]), to_rgb=True) #RGB
train_pipeline = [
    dict(type='LoadImagePairFromFile', spectrals=('visible', 'thermal'), daynight=True),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'train_with_occ_1_class_cleaned_with_cyclist_and_people_cleaned_new.json',
        img_prefix=data_root + 'train/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'val_with_occ_1_class_cleaned_with_cyclist_and_people_cleaned_new.json',
...
checkpoint_config = dict(  # Checkpoint hook 的配置文件。执行时请参考 https://github.com/open-mmlab/mmcv/blob/master/mmcv/runner/hooks/checkpoint.py。
    interval=1)  # save checkpoint interval。
log_config = dict(  # register logger hook 的配置文件。
    interval=50,  # logging interval
    hooks=[
...
log_level = 'INFO'
# load_from = '/home/zhangyan22/mmdetection/checkpoints/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'
load_from = None
resume_from = None
workflow = [('train', 1)]  # runner 的工作流程，[('train', 1)] 表示只有一个工作流且工作流仅执行一次。根据 total_epochs 工作流训练 12个回合。
work_dir = 'work_dirs/faster_rcnn_r50_fpn_tinet/kaist'